...
params_tuples = [
    ...
    pytest.param({"filter": "76", "username": "123"}, id="with_params"),
]


@pytest.mark.parametrize("params", params_tuples)
def test_middleware_for_authenticated_user(db, client, params, caplog):
    ...
    response = client.get(add_url_params("/path", params | {"auto_login": generate_jwt(user)}))
